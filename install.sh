...
main() {
  ...
  # ── Generate run.sh (restart script) ──────────────────────────────────────
  local run_script_path="$install_dir/run.sh"
  local generated_at
  generated_at="$(date -u +'%Y-%m-%dT%H:%M:%SZ')"
  info "Generating restart script: $run_script_path"

  if [[ "$tunnel_choice" == "1" ]]; then
    cat > "$run_script_path" <<RUNSH
#!/usr/bin/env bash
# Whiteboard Agent — restart script
# Generated by install.sh on ${generated_at}
# Usage: ${run_script_path}
#
# Re-launches the companion behind a fresh temporary Cloudflare tunnel.
# The trycloudflare.com URL changes on every run.
...
# Usage: ${run_script_path}
#
# Re-launches the companion against your permanent public URL.
...
RUNSH
  ...
  if ! bash -n "$run_script_path"; then
    err "Generated run.sh failed syntax check: $run_script_path"
    exit 1
  fi
  ok "Restart script ready: $run_script_path"
