...
function startQuickTunnel(targetUrl, opts) {
  ...
  // `wendy start` stays alive as a long-running process for the entire
  // tunnel lifetime, so the parent process never exits while the tunnel
  // is active. This means we can safely use pipes for stdout/stderr —
  // no SIGPIPE will occur because the read-end (parent) is always open.
  // We do NOT call child.unref(): the tunnel is intentionally tied to
  // the wendy start process lifecycle.
  return new Promise(function (resolve, reject) {
    let child;
    try {
      // Pass an explicit empty config so cloudflared ignores the user's
      // ~/.cloudflared/config.yml. If that file exists (e.g. from a named
      // tunnel that routes n8n or another service) and contains a catch-all
      // `http_status:404` ingress rule, cloudflared loads it even in --url
      // mode and all quick-tunnel traffic returns 404.
      // /dev/null on Unix, NUL on Windows — both read as an empty file with
    ...
    const finish = function (url) {
      ...
      resolved = true;
      clearTimeout(timer);
      // Do NOT unref — the caller (wendy start) stays alive and owns
      // this process for its entire tunnel lifetime.
