...
function startQuickTunnel(targetUrl, opts) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    const finish = function (url) {
      ...
      resolved = true;
      clearTimeout(timer);
      // Destroy the stdout/stderr pipes before unref-ing the child. When the
      // parent process exits, open pipe file-descriptors would be closed by the
      // OS, which sends SIGPIPE to cloudflared the next time it writes — killing
      // it. Destroying the streams here severs that link so cloudflared can keep
      // running independently after the parent exits.
      try { if (child.stdout) child.stdout.destroy(); } catch (_) { /* ignore */ }
      try { if (child.stderr) child.stderr.destroy(); } catch (_) { /* ignore */ }
      try {
        child.unref();
      } catch (_) {
        /* ignore */
